...
void VMTest::run_and_compare(std::string code, std::string out, std::string err) {
...
bool VMTest::run_code(std::string s) {
    int asm_err = tx_asm_run_assembler_buffer(&as, (char*)s.c_str(), s.length());
    if (asm_err != 0) {
