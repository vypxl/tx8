...
program:     %empty 
           ...
           | program tx_asm_EOL
           | program statement
statement:   tx_asm_LABEL tx_asm_SPACE instruction        { tx_asm_assembler_new_label(tx_asm_yyasm, $1); tx_asm_assembler_set_label_position(tx_asm_yyasm, $1); free($1); tx_asm_assembler_add_instruction(tx_asm_yyasm, $3); }
           | tx_asm_LABEL tx_asm_EOL                      { tx_asm_assembler_new_label(tx_asm_yyasm, $1); tx_asm_assembler_set_label_position(tx_asm_yyasm, $1); free($1); }
...
parameter:   tx_asm_ALIAS                                 { tx_asm_Parameter p = { .value.u = 0xdeadbeef, .mode = tx_param_constant32 }; $$ = p; free($1); }
           ...
           | tx_asm_LABEL                                 { tx_asm_Parameter p = { .value.u = tx_asm_assembler_new_label(tx_asm_yyasm, $1), .mode = tx_asm_param_label_id }; $$ = p; }
%%
