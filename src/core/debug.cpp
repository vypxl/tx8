...
void tx::debug::print_raw_instruction(const tx_Instruction& inst) {
    tx::log_err(
        "{:x} ({}) {:x} {:x} (modes: {:x} {:x})\n",
...
void tx::debug::print_instruction(const tx_Instruction& inst) {
...
void tx::debug::print_parameter(const tx_Parameter& p) {
    tx_num32 v = p.value;
    switch (p.mode) {
